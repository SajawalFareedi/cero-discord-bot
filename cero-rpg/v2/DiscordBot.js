...
class DiscordBot extends BaseHelper {
  ...
  loadEventListeners() {
    ...
    this.bot.on("message", async (message) => {
      ...
      if (
        ...
        message.channel.parent.name.toLowerCase() !== "cero-rpg"
      ) {
        return;
      }
    ...
    this.bot.on("guildDelete", async (guild) => {
      guild.members.cache
        .filter((member) => this.onlinePlayers.has(member.id + member.guild.id))
        .forEach((member) =>
    ...
    this.bot.on("presenceUpdate", async (oldM, newM) => {
      ...
      if (!newMember.user.bot) {
        if (newM.status !== "offline") {
          if (
            await this.Game.dbClass().shouldBeInList(
              newMember.id,
              newMember.guild.id
            )
          ) {
    ...
    this.bot.on("guildMemberAdd", async (member) => {
      ...
      const welcomeChannel = await member.guild.channels.cache.find(
        (channel) => channel.name === "newcomers" && channel.type === "text"
      );
      if (welcomeChannel) {
        ...
        welcomeChannel.send(
          `Welcome ${member}! This server has an cero-rpg bot! If you have any questions check the <#${
            member.guild.channels.cache.find(
              (channel) => channel.name === "faq" && channel.type === "text"
            ).id
          }> or PM me !help.`
        );
  ...
  loadHeartBeat() {
    const interval = process.env.NODE_ENV.includes("production")
      ? this.tickInMinutes
      : 1;
      ...
      this.bot.guilds.cache.forEach(async (guild) => {
        ...
        if (process.env.NODE_ENV.includes("production")) {
          ...
          if (guildOnlineMembers.size >= 50) {
            guildMinTimer =
              (Number(minimalTimer) +
                Math.floor(guildOnlineMembers.size / 50)) *
              1000 *
              60;
            guildMaxTimer =
              (Number(maximumTimer) +
          ...
          guild.members.cache
            .map((member) =>
              this.onlinePlayers.get(member.id + member.guild.id)
            )
            .filter(
              (member) => member && !member.timer && guild.id === member.guildId
            )
            .forEach((player) => {
              const playerTimer = this.randomBetween(
                guildMinTimer,
                guildMaxTimer
              );
              player.timer = setTimeout(async () => {
                ...
                if (
                  !this.onlinePlayers.has(player.discordId + player.guildId)
                ) {
                  delete player.timer;
                  return;
                }
                const eventResult = await this.Game.activateEvent(
                  guild.id,
                  player,
                  guildOnlineMembers
                );
  ...
  async processDetails() {
    ...
    console.log(
      `Current Up Time: ${this.secondsToTimeFormat(
    ...
    setTimeout(() => {
      this.bot.guilds.cache.forEach((guild) => {
        ...
        if (actionsChannel) {
          ...
          guildConfig.multiplier--;
          guildConfig.multiplier =
            guildConfig.multiplier - 1 <= 0
              ? (guildConfig.multiplier = 1)
              : guildConfig.multiplier--;
          this.Game.dbClass().updateGame(guild.id, guildConfig);
        }
      });
  ...
  dailyLottery() {
    ...
    this.bot.guilds.cache.forEach(async (guild) => {
      ...
      const randomWinner = await this.randomBetween(
        0,
        guildLotteryPlayers.length - 1
      );
      const winner = guildLotteryPlayers[randomWinner];
      const eventMsg = this.setImportantMessage(
        `Out of ${guildLotteryPlayers.length} contestants, ${winner.name} has won the daily lottery of ${guildConfig.dailyLottery.prizePool} gold!`
      );
      ...
      if (guild.id === guildID) {
        ...
        if (lotteryChannel) {
          let lotteryMessages = await lotteryChannel.messages.fetch({
            limit: 10,
          });
      ...
      guildLotteryPlayers.forEach((player) => {
        ...
        if (player.discordId !== winner.discordId && discordUser) {
          discordUser.send(
            `Thank you for participating in the lottery! Unfortunately ${winner.name} has won the prize of ${guildConfig.dailyLottery.prizePool} out of ${guildLotteryPlayers.length} people.`
          );
        } else if (discordUser) {
          discordUser.send(
            `Thank you for participating in the lottery! You have won the prize of ${guildConfig.dailyLottery.prizePool} out of ${guildLotteryPlayers.length} people.`
          );
        }
      });
  ...
  updateLeaderboards() {
    ...
    this.bot.guilds.cache.each(async (guild) => {
      ...
      if (
        !botGuildMember.permissions.has(["VIEW_CHANNEL", "MANAGE_CHANNELS"])
      ) {
        return;
      }
      ...
      if (
        !leaderboardChannel ||
        (leaderboardChannel && !leaderboardChannel.manageable)
      ) {
        return;
      }

      for (let i = 0; i < types.length; i++) {
        const top10 = await this.Game.dbClass().loadTop10(
          types[i],
          guild.id,
          this.bot.user.id
        );
        const rankString = `${top10
          .filter((player) =>
            Object.keys(types[i])[0].includes(".")
              ? player[Object.keys(types[i])[0].split(".")[0]][
                  Object.keys(types[i])[0].split(".")[1]
                ]
              : player[Object.keys(types[i])[0]] > 0
          )
          .sort((player1, player2) => {
            ...
            return (
              player2[Object.keys(types[i])[0]] -
              player1[Object.keys(types[i])[0]]
            );
          })
          .map(
            (player, rank) =>
              `Rank ${rank + 1}: ${player.name} - ${
                Object.keys(types[i])[0].includes(".")
                  ? `${Object.keys(types[i])[0].split(".")[0]}: ${
                      player[Object.keys(types[i])[0].split(".")[0]][
                        Object.keys(types[i])[0].split(".")[1]
                      ]
                    }`
                  : `${Object.keys(types[i])[0].replace(
                      "currentBounty",
                      "Bounty"
                    )}: ${player[Object.keys(types[i])[0]]}`
              }`
          )
          .join("\n")}`;
...
${rankString}\`\`\``;
        ...
        if (
          !msg.includes(msgCount.array()[i].toString()) &&
          msgCount.array()[i].author.id === this.bot.user.id
        ) {
  ...
  blizzardRandom() {
    this.bot.guilds.cache.forEach(async (guild) => {
      ...
      if (blizzardDice <= 15 && !guildConfig.events.isBlizzardActive) {
        let actionChannel = guild.channels.cache.find(
          (channel) =>
            channel && channel.name === "actions" && channel.type === "text"
        );
        if (actionChannel) {
        ...
        await this.Game.dbClass().updateGame(guild.id, guildConfig);
        setTimeout(() => {
          actionChannel = guild.channels.cache.find(
            (channel) =>
              channel && channel.name === "actions" && channel.type === "text"
          );
          if (actionChannel) {
