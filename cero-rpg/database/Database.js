...
class Database {
  ...
  updateGame(guildId, newConfig) {
    return new Promise((resolve, reject) =>
      Game.updateOne({ guildId }, newConfig, (err, result) => {
        if (err) {
          return reject(err);
        }
  ...
  createNewPlayer(discordId, guildId, name) {
    return new Promise((resolve, reject) =>
      Player.create(newPlayerObj(discordId, guildId, name), (err, result) => {
        if (err) {
          return reject(err);
        }
  ...
  loadPvpLog(discordId) {
    return new Promise((resolve, reject) =>
      PvpLog.findOne({ playerId: discordId }, (err, result) => {
        if (err) {
          return reject(err);
        }
        if (!result) {
  ...
  loadMoveLog(discordId) {
    return new Promise((resolve, reject) =>
      MoveLog.findOne({ playerId: discordId }, (err, result) => {
        if (err) {
          return reject(err);
        }
        if (!result) {
  ...
  loadTop10(type, guildId, botID) {
    ...
    const removeNpcs = enumHelper.roamingNpcs
      .map((npc) => npc.discordId)
      .concat(botID);
    enumHelper.mockPlayers
      .map((npc) => npc.name)
      .forEach((npc) => removeNpcs.push(npc));
  ...
  loadCurrentRank(player, type) {
    ...
    query[Object.keys(type)[0]] = {
      $gte: Object.keys(type)[0].includes(".")
  ...
  getSameMapPlayers(guildId, playerMap, selectFields = {}) {
    ...
    return new Promise((resolve, reject) =>
      Player.find({ "map.name": playerMap, guildId }, (err, result) => {
        if (err) {
          return reject(err);
        }
  ...
  deletePlayer(playerId) {
    return new Promise((resolve, reject) =>
      Player.remove({ discordId: playerId }, (err, result) => {
        if (err) {
          return reject(err);
        }
  ...
  resetAllLogs(guildId) {
    return new Promise((resolve, reject) =>
      MoveLog.updateMany({ guildId }, {}, { mulit: true }, (err, result) => {
        if (err) {
          return reject(err);
        }

        return resolve(ActionLog.updateMany({ guildId }, {}, { multi: true }));
      })
    );
  }
  ...
  deleteAllPlayersInGuild(guildId) {
    return new Promise((resolve, reject) =>
      Player.remove({ guildId }, (err, result) => {
        if (err) {
          return reject(err);
        }
