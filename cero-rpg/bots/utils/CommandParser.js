  ...
  async parseUserCommand(messageObj) {
    try {
      const commandChannelId =
        (await messageObj.channel.type) === "dm"
          ? messageObj.channel.type
          : messageObj.guild.channels.cache.find((channel) => {
              if (
      ...
      const guildId =
        messageObj.channel.type === "dm"
          ? (await this.Game.Database.getPlayerGuildId(messageObj.author.id))
              .guildId
          : messageObj.guild.id;
      ...
      if (commandRegex.test(command)) {
        const commandObj = await commands.filter((c) =>
          c.command instanceof Array
            ? c.command.includes(command)
            : c.command === command
        )[0];
        if (!commandObj) {
          return;
        ...
        commandLog.info({
          ...
          command: messageObj.content,
          time: moment().utc().toISOString(),
        });

        if (
          commandObj.channelOnlyId &&
          ...
          return messageObj
            ...
            .then(
              messageObj.author.send(
                `Please send this to <#${commandChannelId}> or PM me.`
              )
            );
        }
        ...
        if (commandObj.operatorOnly && !botOperators.includes(authorId)) {
          return messageObj
            .delete(1500)
            .then(
              messageObj.author.send("This is a bot operator only command.")
            );
        }
        const params = {
          Game: this.Game,
      ...
      if (
        messageObj.content.startsWith(guildPrefix) &&
        channelId !== commandChannelId
      ) {
        return messageObj.author.send(
          `Please check ${guildPrefix}help for more info. ${messageObj.content} was an invalid command.`
        );
      }
    } catch (err) {
      errorLog.error(err);
