...
const processDetails = () => {
  ...
  console.log("------------");
  console.log(
    `\n\nHeap Usage:\n  RSS: ${(memoryUsage.rss / 1048576).toFixed(
      2
    )}MB\n  HeapTotal: ${(memoryUsage.heapTotal / 1048576).toFixed(
      2
    )}MB\n  HeapUsed: ${(memoryUsage.heapUsed / 1048576).toFixed(2)}MB`
  );
  console.log(
    `Current Up Time: ${helper.secondsToTimeFormat(
...
const heartBeat = () => {
  ...
  if (discordUsers) {
    const discordOfflinePlayers = discordUsers
      .filter(
        (player) => player.presence.status === "offline" && !player.user.bot
      )
      .map((player) => {
        return {
            ...
            array.findIndex(
              (p) =>
                ...
                discordOfflinePlayers.findIndex(
                  (offlinePlayer) =>
                    offlinePlayer.discordId === player.discordId
                ) === -1
            )
        );

      onlinePlayerList.forEach((player) =>
        discordOfflinePlayers.filter(
          (offPlayer) => offPlayer.discordId === player.discordId
        )
      );
    }
    ...
    if (onlinePlayerList.length >= 50) {
      console.log(
        `MinTimer: ${minTimer / 1000 / 60} - MaxTimer: ${maxTimer / 1000 / 60}`
      );
      minTimer =
        (Number(minimalTimer) + Math.floor(onlinePlayerList.length / 50)) *
        1000 *
        60;
      maxTimer =
        (Number(maximumTimer) + Math.floor(onlinePlayerList.length / 50)) *
        1000 *
        60;
    }
...
discordBot.on("messageCreate", async (message) => {
  ...
  if (
    process.env.VIRUS_TOTAL_APIKEY &&
    message.attachments &&
...
discordBot.on("guildMemberAdd", (member) => {
  ...
  channel.send(
    `Welcome ${member}! This server has an cero-rpg bot! If you have any questions check the <#${faqChannelId}> or PM me !help.`
  );
