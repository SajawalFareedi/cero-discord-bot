...
class Commands extends aggregation(BaseGame, BaseHelper) {
  constructor(params) {
    super();
    const { Database, Events, MapManager, ItemManager, MonsterManager } =
      params;
  ...
  async playerStats(params) {
    ...
    return !playerToCheck || playerToCheck.id === author.id
      ...
      : author.send(
          result.replace(
            "Here are your stats!",
            `Here are ${loadedPlayer.name}'s stats!`
          )
        );
  }
    ...
    return !playerToCheck || playerToCheck.id === author.id
      ...
      : author.send(
          result.replace(
            ...
            `Here is ${loadedPlayer.name}'s equipment!`
          )
        );
  }
    ...
    return !playerToCheck || playerToCheck.id === author.id
      ...
      : author.send(
          result.replace(
            ...
            `Here\'s ${loadedPlayer.name}'s spellbook!`
          )
        );
  }
  ...
  async resetLottery({ guildId, guild }) {
    ...
    const lotteryChannel = await guild.channels.cache.get(
      enumHelper.channels.lottery
    );
    if (lotteryChannel) {
      ...
      if (lotteryMessages.size <= 0) {
        await lotteryChannel.send(
          "cero-rpg Lottery - You must pay 100 gold to enter! PM me `!lottery` to join"
        );
        await lotteryChannel.send(
          `Current lottery prize pool: ${newPrizePool}`
        );
        ...
        await lotteryMessages
          .array()[1]
          .edit(`Current lottery prize pool: ${newPrizePool}`);
  ...
  async resetQuest(params) {
    ...
    try {
      ...
      let { updatedPlayer } = await this.Events.retrieveNewQuest(
        loadedPlayer,
        true
      );
      if (updatedPlayer.quest.questMob.name === oldQuestMob) {
        const newQuestResult = await this.Events.retrieveNewQuest(
          loadedPlayer,
          true
        );
  ...
  async joinLottery(params) {
    ...
    if (!canJoinLottery) {
      return author.send(
        "Joining lottery is currently disabled, please try again in a few."
      );
    }

    const player = await this.Database.loadPlayer(author.id, {
      pastEvents: 0,
      pastPvpEvents: 0,
    });
  ...
  async prizePool(params) {
    ...
    const lotteryPlayers = await this.Database.loadLotteryPlayers(
      player.guildId
    );
    const guildConfig = await this.Database.loadGame(player.guildId);
  ...
  async listTitles(params) {
    ...
    if (loadedPlayer.titles.unlocked.length <= 0) {
      return author.send(
        "I'm sorry, you currently do not have any titles unlocked."
      );
    }

    return author.send(
      `You currently have ${loadedPlayer.titles.unlocked.join(
        ", "
  ...
  async setTitle(params) {
    ...
    if (loadedPlayer.titles.unlocked.length <= 0) {
      return author.send(
        "I'm sorry, but you have no titles unlocked as of yet."
      );
    }
  ...
  async top10(params) {
    ...
    const loadedTop10 = await this.Database.loadTop10(
      type,
      guildId,
      Bot.user.id
    );
...
${rankString}\`\`\``);
  ...
  getRank(params) {
    ...
    return this.Database.loadPlayer(author.id, {
      pastEvents: 0,
      pastPvpEvents: 0,
    })
        ...
        if (
          ...
          actionsChannel.send(
            this.setImportantMessage(
              `${player.name}${
                player.titles.current !== "None"
                  ? ` the ${player.titles.current}`
                  : ""
              } just cast${
                calcAmount > 1 ? ` ${calcAmount}x ` : " "
              }${spell}!!\nCurrent Active Bless: ${
                guildConfig.spells.activeBless
              }\nCurrent Multiplier is: ${guildConfig.multiplier}x`
            )
          );
          setTimeout(async () => {
            const newLoadedConfig = await this.Database.loadGame(
              player.guildId
            );
            newLoadedConfig.multiplier = Math.max(
              1,
          ...
          author.send(
            `You do not have enough gold! This spell costs ${
              ...
              globalSpells.bless.spellCost - player.gold.current
            } gold.`
          );
        }
        break;
        ...
        if (player.gold.current >= globalSpells.home.spellCost) {
          ...
          actionsChannel.send(
            `${player.name}${
              player.titles.current !== "None"
                ? ` the ${player.titles.current}`
                : ""
            } just cast ${spell} and teleported back to ${randomHome.name}.`
          );
          ...
          author.send(
            `You do not have enough gold! This spell costs ${
              ...
              globalSpells.home.spellCost - player.gold.current
            } gold.`
          );
        }
        break;
  ...
  async placeBounty(params) {
    ...
    return author.send(
      `Bounty of ${amount} placed on ${bountyRecipient.name}'s head!`
    );
  }
  ...
  async setServer(params) {
    ...
    if (
      !confirmation &&
      value !== guildID &&
      loadedPlayer.equipment.relic.name !== "Nothing"
    ) {
      return author.send(
...
• **Is this open source?**
Yes, <https://github.com/sizzlorox/cero-rpg-Bot>

• **Do you guys have a trello board?**
Yes, <https://trello.com/b/OnpWqvlp/cero-rpg>
...
There's a command to get the invite link ${value}invite`);
        ...
        if (message) {
          await Bot.guilds.cache.forEach((guild) =>
            guild.channels.cache
              .find(
                (channel) =>
                  channel.name === "actions" && channel.type === "text"
              )
              .send(message)
          );
          return author.send(
            `Holiday ${whichHoliday} ${whichMessage} message sent`
          );
        }

        return author.send(
          `Holiday ${whichHoliday} ${whichMessage} message failed to send`
        );
      default:
        return author.send(
          `Holiday ${whichHoliday} ${whichMessage} message failed to send`
  ...
  async updateHoliday(params) {
    ...
    if (isStarting) {
      ...
      return author.send(
        `Holiday ${whichHoliday} start message failed to send`
      );
    }
  ...
  async resetPlayers(params) {
    ...
    if (announcementChannel) {
      if (messagePromises.length) {
        await Promise.all(messagePromises).then(
